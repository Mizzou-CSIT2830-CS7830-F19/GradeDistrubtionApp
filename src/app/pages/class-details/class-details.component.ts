...
export class ClassDetailsComponent implements OnInit, OnDestroy {
  ...
  barChartOptions: ChartOptions = {
    ...
    scales: { xAxes: [{ categoryPercentage: 1.0, barPercentage: 1.0 }], yAxes: [{}] }
  };
  ...
  ngOnInit() {
    ...
    this.classDetails = this.dataService.getClassDetails(this.professorId, this.classId);
    this.barChartDataSub = this.classDetails.subscribe(details => {
      if (details !== undefined) {
        this.barChartData = [{ data: [details.a, details.b, details.c, details.d, details.f] }];
      }
    });
  }
